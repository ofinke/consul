...
# @tool
# def get_source_code(
#     target_type: str, name: str, file_path: str | None = None, project_root: str = ".", include_context: bool = False
# ) -> dict[str, Any]:
#     """
...
#         try:
#             with Path.open(file_obj, "r", encoding="utf-8") as f:
#                 content = f.read()
#             return {
#                 "name": file_obj.name,
#                 "type": "file",
...
#     # Search for function/class/method
#     files_to_search = [Path(file_path)] if file_path else list(project_path.rglob("*.py"))

#     for file_path_obj in files_to_search:
#         try:
#             with Path.open(file_path_obj, "r", encoding="utf-8") as f:
#                 content = f.read()
#                 lines = content.splitlines()

#             tree = ast.parse(content)

#             # Find the target in AST
...
#     query: str, project_root: str = ".", file_pattern: str = "**/*.py", context_lines: int = 3, max_results: int = 20
# ) -> dict[str, Any]:
#     """
...
#     try:
#         project_path = Path(project_root).resolve()
#         files = [f for f in project_path.glob(file_pattern) if f.is_file()]
...
#             try:
#                 with Path.open(file_path, "r", encoding="utf-8", errors="ignore") as f:
#                     lines = f.readlines()
